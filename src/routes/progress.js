...
// Joriy foydalanuvchining XP, streak va darajasini olish
router.get("/me", requireAuth, async (req, res) => {
  ...
  const userResult = await pool.query(
    "SELECT full_name, cefr_level FROM users WHERE id = $1",
    [req.userId]
  );
  res.json({
    xp: Number(xpResult.rows[0].total_xp),
    streak: streakResult.rows[0] || { current_streak: 0, longest_streak: 0 },
    user: userResult.rows[0],
  });
});

// Mashq tugagach XP qo'shish va streak'ni yangilash
router.post("/xp", requireAuth, async (req, res) => {
  ...
  const streakRow = await pool.query("SELECT * FROM streaks WHERE user_id = $1", [req.userId]);
  const s = streakRow.rows[0];
  let newStreak = 1;
  if (s && s.last_activity_date) {
    const last = new Date(s.last_activity_date);
...
// Reyting jadvali - top 20 foydalanuvchi
router.get("/leaderboard", requireAuth, async (req, res) => {
  const result = await pool.query(
    `SELECT u.full_name, COALESCE(SUM(x.amount), 0) AS total_xp
     FROM users u
